...
COMMON_PROMPT_FILE = ROOT / "examples/COMMON_PROMPT.txt"
...
COMMON_PROMPT = _read(COMMON_PROMPT_FILE).strip()
...
def test_run_human_ab_explicitly_proves_pwd_and_skill_state() -> None:
    script = _read(RUN_HUMAN_AB)
    assert 'COMMON_PROMPT_FILE="${EXAMPLES_DIR}/COMMON_PROMPT.txt"' in script
    assert 'load_common_prompt() {' in script
    assert 'require_file "${COMMON_PROMPT_FILE}" "A/B COMMON_PROMPT"' in script
    assert 'COMMON_PROMPT="$(cat "${COMMON_PROMPT_FILE}")"' in script
...
def test_common_prompt_is_standalone_file() -> None:
    prompt = _read(COMMON_PROMPT_FILE).strip()

    assert COMMON_PROMPT_FILE.is_file()
...
def test_run_human_ab_replays_current_fix_state_and_validates_exports() -> None:
    ...
    assert 'B 轮导出未引用 bob-pgpool-bad-2026-04-12' in script
