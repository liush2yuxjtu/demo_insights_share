...
#
# ----------------------------------------------------------------
# pkill -f "insights_cli.py serve" 2>/dev/null; rm -rf ~/.claude/skills/insights-share ~/.claude/skills/insights-share-server 2>/dev/null; rm -rf /tmp/demo_insights_A && mkdir -p /tmp/demo_insights_A && cd /tmp/demo_insights_A && claude -p "我们的 checkout API 正在超时，postgres 在午餐高峰拒绝新连接，应该如何诊断与修复？请给出可执行的 SQL 与代码片段。" 2>&1 | sed -E 's/\x1b\[[0-9;]*[A-Za-z]//g' | tee /tmp/demo_insights_A/A_without.log
# ----------------------------------------------------------------
#
# 说明：
#   1. 先停 daemon，再删 ~/.claude/skills/insights-share* 保证环境干净
