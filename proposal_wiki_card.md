...
```json
{
  ...
  "conflict_with": ["bob-pgpool-2026-04-15"],   // 互斥卡片 id 列表
  "conflict_reason": "fix 方向相反：transaction mode vs session mode",
  "supersedes": [],        // 本卡取代了哪些旧卡
  "superseded_by": null    // 本卡被哪张取代（deprecated 时填写）
}
```
...
冲突检测结果写入新字段 `conflict_with`，dashboard 展示冲突队列，由管理员决定：
...
`adapter.py` 消费时：若 top-1 卡带有 `conflict_flag`，在 adapted_insight 末尾追加一行警告：
```
