...
| S10 | LAN daemon（insightsd）| `insights-share/demo_codes/insights_cli.py serve --port 7821` | `/healthz` 200；`/insights` 返回 `{"cards":[...]}`；`/search?q=postgres&k=3` 返回 `{"hits":[...]}`；flat 模式 `/topics` 返 400 `topics_not_supported`（tree 模式才支持） | HTTP 全部 200/400 符合契约；cards 非空 + hits 非空（query 命中）|
