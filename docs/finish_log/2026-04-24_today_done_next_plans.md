...
- `TODOS.md` 当前没有开放 E2E 阻塞项；`UC-1 plugin bundle self-containment` 已关闭。
...
4. 当前没有开放 E2E 阻塞项；默认下一步是 release/PR 收尾，整理本轮证据、确认 diff、提交 PR 或发布收尾。
5. adoption-proof 扩展是后续产品开发轨道：在现有最小 adoption proof 通过的基础上，继续扩展更多真实采纳证据、质量指标和 day-2/day-7 回访信号；它不是当前 E2E 阻塞项。
