...
| O2 layer-skip topics inject | `181bdf2` | PASS | topics.json 注入 PROMPT；max_turns 5→3；metrics.topics_count |
...
| M7 gate（cache-miss） | `bf2c61a` + `b0bc955` | PARTIAL | O2 regression 修复后 re-capture：cache-miss n=5 p50=9918ms / p95=12314ms（budget 6000ms，仍超 105%，gate_p95_budget=FAIL）；cache-hit n=5 p50=1ms / p95=1ms（budget 3000ms，PASS）；5/5 round-1 queries status=ok（O2 `max_turns` 恢复 5 + layer-skip 改为"提示型"不再拼硬路径，`<<<SEARCH_HITS>>>` 全部正常返回真实 hits）；延迟问题退化回与 MVP 基线同量级（post_mvp p95=12299ms vs 现在 12314ms），O2/O4/O6 对 miss 路径没有带来净收益 → 下一步需在 search_agent 层面继续优化（prompt 精简 / 分层检索 / 或直接上 HNSW 索引），而非继续堆 layer-skip hint |
| O7' local_search 短路（M7 gate 二轮） | `87acecf` | PASS | 把 O7 的 embedding 计划降维为零依赖 token overlap：新 `local_search.py` 用 Jaccard + CJK bigram 对 `topics.json` + `<type>/INDEX.md` 候选做预排序；`search_agent.run` 在 top ≥ 0.12 阈值时直接短路，绕开 MiniMax haiku；配套 `adapter.py` 新增 `_local_bypass`：card `source=local` 且分数过阈值时不再跑 adapter SDK。5+5 re-capture（`/tmp/m7_metrics_v2.jsonl`）：cache-miss n=5 p50=24ms / p95=1059ms（budget 6000ms，**18% 预算**）；cache-hit n=5 p50=11ms / p95=15ms（budget 3000ms，0.5% 预算）。`latency_gate.py` 输出 `verdict=PASS`，全部 budget/regression 门绿。样本写入 `samples_post_m7_v2` |
