...
insights-wiki 当前对用户/client **不可感知**：
...
用 Claude Code statusline 持续展示 insights-wiki 工作状态 + 今日累计触发次数，给用户/client 一条**实时信任感信号**。
...
```
[wiki ✓ N/today]    insights-wiki 运行中，今天已触发 N 次
[wiki ✗ 0/today]    daemon/skill 未装或未命中任何 prompt
[wiki … N/today]    命中在途（fetch/verify 中）
```
...
1. `~/.cache/insights-wiki/today_count.json` → today_count + last card
2. `curl -s --max-time 0.3 http://<wiki_host>:7821/health` → daemon 可达性
3. `ls ~/.claude/skills/insights-wiki/SKILL.md` → skill 安装性
...
- A/B 验证场景下 A 侧强制走 `WIKI_STATUSLINE=off`，禁止泄漏 B 特征

## 交付物

1. `statusline/insights_wiki_statusline.sh` - bash 渲染脚本
